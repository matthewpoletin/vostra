...
void AccelInit(void)
{
	...
	ADC1->JSQR |= (uint32_t)12<<(5*1);	// номер канала для первого преобразования
	ADC1->JSQR |= (uint32_t)11<<(5*2);	// номер канала для второго преобразования
	ADC1->JSQR |= (uint32_t)10<<(5*3);	// номер канала для третьего преобразования
